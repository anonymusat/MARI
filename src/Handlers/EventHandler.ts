...
export default class EventHandler {
	...
	handle = async (event: IEvent): Promise<void> => {
			...
			return void (await this.client.sendMessage(
				...
				MessageType.image,
				{
					caption: `- ${group.subject || "___"} -\n\n💠 *Group Description:*\n${
						group.desc
					}\n\nFOLLOW THE RULES OTHERWISE YOU GONNA REGRET HOPE YOU GOT IT?!\n\n*‣ ${event.participants
						.map((jid) => `@${jid.split("@")[0]}`)
						.join(", ")}*`,
					contextInfo,
				}
		...
		if (bye) {
			...
			return void (await this.client.sendMessage(
				...
				MessageType.image,
				{
					caption: `*@${
						event.participants[0].split("@")[0]
					}*PATHETIC*ANYWAYS goodbye hope you come again(never*)(* ￣︿￣).`,
					contextInfo,
				}
			));
		}
		if (promote) {
			const text = `HEY *@${
				event.participants[0].split("@")[0]
			}*, you're now an admin make sure to be an good admin otherwise disadmin button is in owner's hand .`;
		...
		if (demote) {
			const text = `pathetic *@${
				event.participants[0].split("@")[0]
			}* you cant even become an good admin *baka😒🙄.`;
			return void this.client.sendMessage(
				event.jid,
				text,
				MessageType.extendedText,
